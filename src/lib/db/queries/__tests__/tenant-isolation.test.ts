...
} from "@/lib/db/__tests__/test-harness";
...
import { findVendorByName, listVendorsByAccount } from "@/lib/db/queries/vendors";
import {
  listAccountUsers,
...
describe("queries/savings", () => {
  it("listSavingsForAccount / getSavingsTotals only return the scoped account", async () => {
    const { db } = await import("@/lib/db");
    const { savingsRecordsTable } = await import("@/lib/db/schema");
...
describe("queries/approvals", () => {
  it("listPendingApprovals + countPendingApprovals only see the scoped account", async () => {
    const { db } = await import("@/lib/db");
    const { renewalEventsTable } = await import("@/lib/db/schema");
...
describe("coverage", () => {
  it("every file under src/lib/db/queries/ has a corresponding describe() in this file", () => {
    const queriesDir = path.join(process.cwd(), "src/lib/db/queries");
    const moduleFiles = readdirSync(queriesDir).filter(
      (f) => f.endsWith(".ts") && !f.endsWith(".d.ts")
    );

    // This test reads its own file content to look for `describe("queries/<name>"`
    // blocks. Cheap, no AST needed.
    const selfPath = path.join(
      process.cwd(),
      "src/lib/db/queries/__tests__/tenant-isolation.test.ts"
    );
    ...
    expect(missing, `Add a describe("queries/<name>") for: ${missing.join(", ")}`).toEqual([]);
  });
});
